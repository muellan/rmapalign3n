...
void show_query_parameters(std::ostream& os, const database& db, const query_options& opt)
{
    ...
    os << comment
       << "nucleotide conversion: " << s.conversion_original()
       << " -> " << s.conversion_replacement() << '\n'
       << comment
       << "k: " << int(s.kmer_size()) << '\n'
       << comment
...
void show_statistics(std::ostream& os,
                     const mapping_statistics& stats,
                     const std::string& prefix)
{
...
void print_static_properties(const database& db)
{
    ...
    std::cout
        << "------------------------------------------------\n"
        << "RmapAlign3N version     " << RMA_VERSION_STRING << " (" << RMA_VERSION << ")\n"
        << "database version     " << RMA_DB_VERSION << '\n'
        << "------------------------------------------------\n"
        ...
        << "nucl. conversion     " << db.target_sketcher().conversion_original() << " -> "
                                   << db.target_sketcher().conversion_replacement() << '\n'
        << "------------------------------------------------\n"
