...
#ifndef RMA_SKETCHERS_H_
#define RMA_SKETCHERS_H_
...
public:
    ...
    single_function_unique_min_hasher(hasher hash = hasher{}):
        ...
        windowSize_(64), windowStride_(64-k_+1),
        convOrig_{'C'}, convRepl_{'T'}
    {}
    ...
    //---------------------------------------------------------------
    /** @return window stride for sketching */
    char conversion_original () const noexcept { return convOrig_; }
    char conversion_replacement () const noexcept { return convRepl_; }
    //-----------------------------------------------------
    /** @brief set window stride for sketching */
    void conversion_rule (char orig, char repl) {
        convOrig_ = orig;
        convRepl_ = repl;
    }
                    ...
                    Consumer&& consume) const
    {
        for_each_window(first, last, windowSize_, windowStride_,
            [&] (InputIterator first, InputIterator last) {
                ...
                for_each_unambiguous_converted_kmer_2bit<kmer_type>(
                    k_, convOrig_, convRepl_, first, last,
                    [&] (kmer_type kmer) {
                        auto h = hash_(kmer);
                        if (h < sketch.back()) {
    ...
    write_binary(std::ostream& os, const single_function_unique_min_hasher& h)
    {
        ...
        write_binary(os, h.convOrig_);
        write_binary(os, h.convRepl_);
    }
    ...
    read_binary(std::istream& is, single_function_unique_min_hasher& h)
    {
        ...
        read_binary(is, h.convOrig_);
        read_binary(is, h.convRepl_);
    }
